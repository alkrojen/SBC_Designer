...
parts of a **phase-1 Sandwich Circuit Board (SCB)** for either side of the PCB,
following the SCB project description, revision F, section 4.

![Demo PCBA with all generated SCB parts, exploded](docs/demo_scb_exploded.png)
...
file is generated by `python -m scb_designer.demo_pcba`.
...
Stand-alone build: download the `SCB_Designer-windows` artifact from the latest
**CI** run on GitHub (Actions tab), unzip it, and run `SCB_Designer.exe`.

From source (64-bit Python 3.10–3.12):
```bat
install_windows.bat        :: creates .venv and installs the dependencies
run_scb_designer.bat       :: starts the application
build_windows_exe.bat      :: optional: builds dist\SCB_Designer\SCB_Designer.exe
```

Command line (batch generation without the GUI):
```bat
.venv\Scripts\python -m scb_designer cli info demo\ac_controller_pcba.step
.venv\Scripts\python -m scb_designer cli generate board.step --part alignment --side bottom -o bottom_alignment.step
.venv\Scripts\python -m scb_designer cli generate board.step --all --out-dir parts
.venv\Scripts\python -m scb_designer cli screenshot board.step --with-parts -o view.png
```
...
* Through-hole parts are enclosed (clearance pocket + sealed dome), but their
  electrical contact is not part of the SCB pressure-contact scheme.
